...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2020/12/04 14:12:54 by jubonill          #+#    #+#             */
/*   Updated: 2020/12/22 13:44:05 by jubonill         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		get_next_line(int fd, char **line)
{
	...
	if (!line || fd >= OPEN_MAX || BUFFER_SIZE < 1 || BUFFER_SIZE >= INT_MAX\
		|| !(buff = (char *)malloc(sizeof(char) * (BUFFER_SIZE + 1))))
		return (-1);
	while ((b_read = read(fd, buff, BUFFER_SIZE)) > 0)
	{
		buff[b_read] = '\0';
		s[fd] = get_buffer(&s[fd], buff);
		if (ft_strrchr(s[fd], '\n') != NULL)
			break ;
	}
	free_and_update(&buff, NULL);
	return (solve(b_read, &s[fd], line));
}
